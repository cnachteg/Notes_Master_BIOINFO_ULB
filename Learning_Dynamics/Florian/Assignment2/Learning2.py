...
for r in range (realization): #nbre de répétitions du test
    ...
    for t in range(tour): #nbre de tours dans un même test
            ...
            if 0 < index1 < 19:
                for index2,y in enumerate(x):
                    if 0< index2 < 19:
                        ...
                        result = int(board1[index1][index2-1]) + int(board1[index1+1][index2]) + int(board1[index1-1][index2])
                        Calculer_W_dans_board_W(index1,index2,board1,board_W1) 
                    elif index2 == 0:
                        result = int(board1[index1][index2+1]) + int(board1[index1+1][index2]) + int(board1[index1-1][index2])
                        Calculer_W_dans_board_W(index1,index2,board1,board_W1)
                ...
                for index2,y in enumerate(x):
                    if 0< index2 < 19:
                        result = int(board1[index1][index2-1]) + int(board1[index1][index2+1]) + int(board1[index1-1][index2])
                        Calculer_W_dans_board_W(index1,index2,board1,board_W1)   
                    elif index2 == 19:
                        result = int(board1[index1][index2-1]) + int(board1[index1-1][index2])
                        Calculer_W_dans_board_W(index1,index2,board1,board_W1) 
                    elif index2 == 0:
                        result = int(board1[index1][index2+1]) + int(board1[index1-1][index2])
                        Calculer_W_dans_board_W(index1,index2,board1,board_W1)
            ...
            if 0 < index1 < 19:
                for index2,y in enumerate(x):
                    if 0< index2 < 19:
                        ...
                        Wi = float(board_W1[index1][index2])
                        ind1, ind2 = random.choice([(index1,index2 - 1),(index1 +1,index2),(index1 -1,index2)])
                        Wj = float(board_W1[ind1][ind2])
                        change_value_board(board1,board2,index1,index2)
                    elif index2 == 0:
                        Wi = float(board_W1[index1][index2])
                ...
                for index2,y in enumerate(x):
                    if 0< index2 < 19:
                        Wi = float(board_W1[index1][index2])
                        ind1, ind2 = random.choice([(index1,index2 + 1),(index1,index2 - 1),(index1 -1,index2)])
                        Wj = float(board_W1[ind1][ind2])
                        change_value_board(board1,board2,index1,index2)   
                    elif index2 == 19:
                        Wi = float(board_W1[index1][index2])
                        ind1, ind2 = random.choice([(index1,index2 - 1),(index1 -1,index2)])
                        Wj = float(board_W1[ind1][ind2])
                        change_value_board(board1,board2,index1,index2) 
                    elif index2 == 0:
                        Wi = float(board_W1[index1][index2])
        ...
        if t == 0 or t == 5 or t == 10 or t == 15 or t == 20 or t == 30 or t == 40 or t == 50 or t == 60:
            print(str(total_cooperators/4)+ "%", t)
            print_board(board1)#Pour imprimer les matrices de fin
            print ("***********")
