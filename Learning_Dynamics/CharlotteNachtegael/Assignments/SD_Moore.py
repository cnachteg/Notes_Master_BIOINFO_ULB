"""
Stag-Hunt game with Moore neighbours
"""
...
def update_strat_moore(lattice,strategy,size,payoff):
    new_strategy = np.zeros((size,size))
    for i in range(size):
        for j in range(size):
            a,b = random.choice(get_neighbours_moore(i,j,size))
            prob_to_update = (1 + (lattice[a][b] - lattice[i][j])/(8*(max(payoff) - min(payoff))))/2
...
def final_distribution(final_coop,size):
    ...
    fig.suptitle("Distribution of final cooperation levels of\nStag-Hunt game with Moore "
                 "neighbours\nLattice of size %sx%s & Replicator" % (size,size), fontsize=14,
                 fontweight='bold')
...
def main(size,payoff):
    ...
    for i in range(500):
        ...
        strategy = update_strat_moore(lattice,strategy,size,payoff)
